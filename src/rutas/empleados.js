...
router.post('/ingresodatos', async (req, res) => {
  ...
  try {
    const rows = await pool.query('INSERT INTO datos_ingreso(cargo, departamento, proyecto, sueldo) VALUES (?, ?, ?, ?)', [cargo, departamento, proyecto, sueldo]);
    return res.send({
...
router.delete('/eliminar/:id', async (req, res) => {
  try {
    const result = await pool.query('DELETE FROM datos_ingreso WHERE idDato = ?', [req.params.id])
    console.log(result);
...
router.patch('/actualizar/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query('UPDATE datos_ingreso SET cargo = IFNULL(?, cargo), departamento = IFNULL(?, departamento), proyecto = IFNULL(?, proyecto), sueldo = IFNULL(?, sueldo) WHERE idDato = ?', [cargo, departamento, proyecto, sueldo, id])

    if(result.affectedRows === 0) return res.status(404).json({message: 'No encontrado'})

    const rows = await pool.query('SELECT * FROM datos_ingreso WHERE idDato = ?', [id])
    console.log(result);
    res.json(rows[0])
  } catch (error) {
    res.status(500).json({
